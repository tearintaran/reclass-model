...
class EvidenceEvent:
    ...
    def signed_points(self) -> float:
        ...
        if self.points is not None:
            ...
            magnitude = float(C.STRENGTH_POINTS[self.applied_strength])
...
# Classification                                                              #
# --------------------------------------------------------------------------- #
def classify(evidence: List[EvidenceEvent], engine_version: str = C.ENGINE_VERSION) -> Classification:
...
# --------------------------------------------------------------------------- #
def _revel_to_event(score: float) -> Optional[EvidenceEvent]:
    """Map a continuous REVEL score to a PP3 or BP4 event at a calibrated strength."""
    for threshold, strength in C.REVEL_PP3:  # high -> low
        if score >= threshold:
    ...
    for threshold, strength in C.REVEL_BP4:  # low -> high
        if score <= threshold:
            return EvidenceEvent(
...
def _gnomad_af_to_event(af: float) -> Optional[EvidenceEvent]:
...
def derive_criteria_from_signals(signals: Dict[str, Any]) -> List[EvidenceEvent]:
    ...
    if "revel" in signals and signals["revel"] is not None:
        ev = _revel_to_event(float(signals["revel"]))
        if ev:
            events.append(ev)

    if "gnomad_af" in signals and signals["gnomad_af"] is not None:
        ev = _gnomad_af_to_event(float(signals["gnomad_af"]))
        if ev:
            events.append(ev)
...
def classify_signals(signals: Dict[str, Any], engine_version: str = C.ENGINE_VERSION) -> Classification:
    """Convenience: signals -> evidence -> classification."""
    return classify(derive_criteria_from_signals(signals), engine_version=engine_version)
